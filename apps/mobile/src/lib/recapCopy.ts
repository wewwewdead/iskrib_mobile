import type {WeeklyRecapPersonal} from './api/analyticsApi';

export const getPostsCopy = (count: number): string => {
...
export const getWordsCopy = (count: number): string => {
...
export const getReactionsCopy = (count: number): string => {
...
export const getViewsCopy = (count: number): string => {
...
const getNextMilestone = (count: number): number | null => {
...
export const getOverallEncouragement = (
  personal: WeeklyRecapPersonal | null,
  streakCount: number,
  ...
  if (gap === 3 && next && next >= 30)
    return `Three days from ${next}. Stay locked in.`;
  ...
  if (streakCount === 30)
    return 'A full month of writing. That changes a person.';
  if (streakCount === 14)
    return 'Two weeks straight. This is becoming who you are.';
  if (streakCount === 7) return 'One week down. The habit is forming.';

  // Freeze awareness
  if (freezeAvailable && streakCount >= 7)
    return `${streakCount} days strong, with a safety net ready.`;

  // General streak tiers
  if (streakCount > 100)
    return `${streakCount} days. At this point, the streak has a streak.`;
  if (streakCount >= 30)
    return "Don't break the chain \u2014 your consistency is legendary.";
  if (streakCount >= 14)
    return 'Deep in the rhythm now. Your readers can feel it.';
  if (streakCount >= 7)
    return "Don't break the streak \u2014 your consistency is paying off.";
  if (streakCount >= 3) return 'Building momentum. Three days and counting.';
  if (streakCount >= 1)
    return 'The streak begins. Come back tomorrow to keep it alive.';
  ...
  if (posts === 0)
    return 'A quiet week, sometimes the best ideas need silence first.';
